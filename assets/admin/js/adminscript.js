...
$(document).ready(function () {
    // alert(1);
    // var base_url = 'http://localhost/codeigniter/woodsberg-website/';
    ...
    $('#save_category').click(function (e) {
        ...
        $.ajax({
            ...
            error: function (xhr) {
                $('#response').html('<p>Network issue: ' + xhr
                    .responseText +
                    '</p>');
            }
        });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if(data.success == 'success'){
                    setTimeout(function () {
                        $('#successModal .modal-body').text('category deleted successfully');
                        $('#successModal').modal('show');
    ...
    $(document).ready(function () {
                    ...
                    success: function (response) {
                        if (response.success === 'success') {
                            ...
                            setTimeout(function () {
                                ...
                                $('#successModal .modal-body').text('Product added successfully');
                                $('#successModal').modal('show');
                                // Delay the redirect to allow modal to show for 1.5 seconds
                                setTimeout(function () {
        ...
        function clearAllErrorMessages() {
            ...
            $('#seasonal_percentage_error').html('');
            $('#product_export_price_error').html('');
            $('#select_product_error').html('');
            $('#product_price_error').html('');
            $('#erp_product_name_error').html('');
            $('#product_stock_error').html('');
            $('#product_code_error').html('');
            $('#product_width_error').html('');
            $('#product_height_error').html('');
        }
        ...
        // Map of error fields to their corresponding HTML elements
            var errorFields = {
                ...
                'seasonal_percentage': '#seasonal_percentage_error',
                'product_export_price': '#product_export_price_error',
                'select_product': '#select_product_error',
                'product_price': '#product_price_error',
                'erp_product_name': '#erp_product_name_error',
                'product_stock': '#product_stock_error',
                'product_code': '#product_code_error',
                'product_width': '#product_width_error',
                'product_height': '#product_height_error'
            };
    ...
    $(document).on('click', '.edit_product', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                console.log(response.data);
                console.log(response.item_code, response.shipping);
                if (response.success === 'success') {
                    ...
                    $('#product_export_price').val(response.data.export_price);
                    // $("#select_product").val(response.data.ttype);
                    ...
                    $('#erp_product_name').val(response.data.erp_product_name);
                    $('#product_codee').val(response.data.item_code);
                    $('#product_shippingg').val(response.data.shipping);
                    $('#product_width').val(response.data.width);
                    $('#product_height').val(response.data.height);

                    // $('#product_code').val(response.data.item_code);
                    // $('#product_shipping').val(response.data.shipping);
                    // $('#product_stock').val(response.data.stock);
                    $('#product_stock').val(response.data.stock);
                    $('#previewImage').attr('src', response.data.image && response.data.image !== '' ? 'uploads/' + response.data.image : 'path/to/no-image.jpg');
                    $('#previewImage1').attr('src', response.data.image1 && response.data.image1 !== '' ? 'uploads/' + response.data.image1 : 'path/to/no-image.jpg');
                    $('#previewImage2').attr('src', response.data.image2 && response.data.image2 !== '' ? 'uploads/' + response.data.image2 : 'path/to/no-image.jpg');
                    $('#previewImage3').attr('src', response.data.image3 && response.data.image3 !== '' ? 'uploads/' + response.data.image3 : 'path/to/no-image.jpg');
                    ...
                    $('#seasonal_percentage').val(response.data.seasonal_percentage);
                    ...
                    if (response.data.is_seasonaloffer == 1) {
                        ...
                        $('.seasonal-percent').removeClass('d-none');    
                      
                      
                    }
                    else {
                        $('#is_seasonaloffer_edit').prop('checked', false);
                         $('.seasonal-percent').addClass('d-none');  
                    }
                    ...
                    $('#previewImage3').attr('src', 'path/to/no-image.jpg');
                    // alert('tax data not found!');
                }
            }
        })
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success === 'success') {
                    ...
                    if(response.errors.product_export_price){
                        $('#product_export_price_edit_error').html(response.errors.product_export_price);
                    }
                    else{
                        $('#product_export_price_edit_error').html('');
                    }


                    // if(response.errors.seasonal_percentage){
                    //     $('#seasonal_percentage_error').html(response.errors.seasonal_percentage);
                    // }
                    // else{
                    //     $('#seasonal_percentage_error').html('');
                    // }
    ...
    $(document).on('click', '#out_of_stock_edit', function () {
        if ($(this).is(':checked')) {
            ...
            $('#out_of_stock_edit_hidden').val(0);
...
$(document).ready(function () {
    $('#apply_coupon').click(function (e) {
        e.preventDefault();
        let couponcode = $('#coupons_code').val();
        let totalprice = $('#product_total').val();
        let formData = new FormData();
        formData.append('coupons_code', couponcode);
        ...
        // console.log($('#coupons_code').val());  // Check this first
        
    //  var couponCode = $('input[name="coupons_code"]').val();
    //  var totalPrice = $('#total_price').val();
    //   if (couponCode === '') {
    //     $('#coupons_code_error').text('Please enter a coupon code');
    //     return;
    // } else {
    //     $('#coupons_code_error').text('');
    // }
    //     alert(couponCode);
    //     alert(totalPrice);
        // var totalPrice = $('#total_price').val();
        // let formData = new FormData($('#apply_coupon_form')[0]);
        // let formData = new FormData();
        // formData.append('coupons_code', $('#coupons_code').val());
        ...
        $.ajax({
            ...
            success: function (response) {
                console.log(response);
                if (response.success) {
                    $('#coupons_code_error').html('').css('color', 'green').text(response.message);
                    $('.success-msg').removeClass('d-none');
                    $('.total').removeClass('d-none');
                    $('.couponcode').text('₹'+ response.discount);
                    $('.totalamount').text(response.final_amount);
                    // $('#total_amount').val(response.final_amount);
                    ...
                    if (response.errors && response.errors.coupons_code) {
                        ...
                        $('#coupons_code_error').html('Unknown error');
                    }
                }
            }
        });
    ...
    $(document).on('click', '#is_edit_footer_category', function () {
        ...
        function update_shipping_charge(user_token) {
        $.ajax({
            url: base_url + 'cart/update_shipping_charge',
            type: 'POST',
            data: {
            ...
            success: function (response) {
                if (response.success === 'success') {
                    $('#shipping_charge').text(response.shipping_charge);
                }
            }
        });
    }
    ...
    $(document).on('click', '.cart-increment-btn', function () {
        ...
        var totalPrice = newQty * unitPrice;
        //row.find('.price-cart').text("₹" + totalPrice.toFixed(2)).attr('data-price', totalPrice); // ✅ only update 
        var quantity = newQty;
                    ...
                    if (newQty <= current_available_stock) {
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                update_shipping_charge(1); // update shipping charge
        ...
        if (currentQty <= 1) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    location.reload(); // optional if you update UI manually
                },
                error: function (xhr, status, error) {
                    console.error("Delete AJAX Error: " + status + ": " + error);
                }
            });
            return; // stop here, don't run update request
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success === 'success') {
                    ...
                    // Coupon & shipping logic
                    if (response.coupon_code || response.shipping_charge != 0) {
                        // $('#coupon_code').html('-' + response.coupon_code);
                        $('#shipping_charges').html(response.shipping_charge);
                        $('.shippingcharges').removeClass('d-none');
                        $('.couponcode').removeClass('d-none');
                    } else {
                        $('#coupon_code').html('');
                        $('#shipping_charges').html('');
                        $('.shippingcharges').addClass('d-none');
    ...
    //         success: function (response) {
    //             console.log(response);
    //             // console.log(response.shipping_charge); // should log 223
    //     // $('#shipping_charges').html(response.shipping_charge);
    //             if (response.success === 'success') {
    //                 $('#orders tbody').html(response.html);
    //                 $('#total_price').html(response.total_price);
    ...
    //                 }
    //                 else{
    //                      $('#coupon_code').html('');       // hide coupon if condition fails
    //                      $('#shipping_charges').html('');  // hide shipping if condition fails  
    //                      $('.shippingcharges').addClass('d-none');
    //                      $('.couponcode').addClass('d-none');
    //                 }
    ...
    $(document).ready(function () {
    function getFilterValuesAndSearch() {
        var date = $('#retail-date').val();
        var ordertype = $('#order-type').val();
        // alert(ordertype);
        var name = $('#retail-name').val();
        var phone = $('#retail-phone').val();
        loadRetailReport(date, name, phone, ordertype);
    }

    $('#retail-date').on('change', getFilterValuesAndSearch);
    $('#retail-name').on('change', getFilterValuesAndSearch);
    $('#retail-phone').on('change', getFilterValuesAndSearch);
    $('#order-type').on('change', getFilterValuesAndSearch);
    
    // ✅ Trigger search immediately on page load
    getFilterValuesAndSearch();
});

    function loadRetailReport(date,name,phone,ordertype) {
...
//MARK: summary report 
    ...
    $(document).ready(function () {
    function getFilterValuesAndSummary() {
        var fromdate = $('#summary-from-date').val();
        var todate = $('#summary-to-date').val();
        loadSummaryReport(fromdate,todate);
    }

    $('#summary-from-date').on('change', getFilterValuesAndSummary);
    $('#summary-to-date').on('change', getFilterValuesAndSummary);
    
    // ✅ Trigger search immediately on page load
    getFilterValuesAndSummary();
});

    function loadSummaryReport(fromdate,todate) {
        // console.log("Fetching report for:", date);

        $.ajax({
            url: base_url + 'admin/reports/getSummaryReport',
            type: 'POST',
            data: { 
                fromdate: fromdate,
                todate: todate
            ...
            success: function (response) {
                console.log(response);
                if (response.success) {
                    $('#summary-report').html(response.html);
                    $('.summary_total_amount').html('Total Amount:' + response.total_amount);
                } else {
                    $('#summary-report').html('<tr><td colspan="4">No data</td></tr>');
                }
            },
            error: function (xhr, status, error) {
                console.error("AJAX error:", error);
                $('#summary-report').html('<tr><td colspan="4" class="text-danger">Error loading data.</td></tr>');
            }
        });
    }
    ...
    //     var $qtySpan = $portfolio.find('[data-qty]');
    //     var $stock = $portfolio.find('#product_stock').val();
    //     // alert($stock)
    //     var currentQty = parseInt($qtySpan.text());
    ...
    //                 if (newQty < stock) {
    //                     // alert('success');
    //                      // Only increment if currentQty is strictly less than stock
...
//  alert(product_id);
    ...
    var quanity = $row.find('#wishlist_quantity').val();
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success === 'success') {
                ...
                if (stock > quanity) {
                    // Step 2: Product in stock → move to cart
                    $.ajax({
    ...
    //MARK: Add dealer
    $('#add_dealer').click(function (e) 
    {
     
        let formData = new FormData($('#adddealer')[0]); // Capture form data
        $.ajax
        ({
            url: base_url + 'admin/Settings/adddealer',
            type: 'POST',
            data: formData,
            dataType: 'json',
            processData: false,
            ...
            success: function (response) {
                ...
                if (response.success === 'success') 
                {
                        ...
                        $('#adddealer')[0].reset();
                        $('#dealer_name_error').html('')
                        $('#dealer_address_error').html('')
                        $('#dealer_phone_error').html('');
                        $('#dealer_username_error').html('');
                        $('#dealer_password_error').html('');
                        $('#dealer_email_error').html('');
                       
                    setTimeout(function () {
                        $('#successModal .modal-body').text('Dealer added successfully');
                        $('#successModal').modal('show');
                        $('#add-retailer').modal('hide');
                        setTimeout(function () {
                            $('#successModal').modal('hide');
                            location.reload(); // This reloads the whole page
                        }, 1000)
                ...
                else 
                {
                    ...
                    if (response.errors.duplicate) {
                        ...
                        if (response.errors.dealer_name) {
                            $('#dealer_name_error').html(response.errors.dealer_name);
                        }
                        else {
                            $('#dealer_name_error').html('');
                        }

                        if (response.errors.dealer_address) {
                            $('#dealer_address_error').html(response.errors.dealer_address);
                        }
                        else {
                            $('#dealer_address_error').html('');
                        }

                        if (response.errors.dealer_phone) {
                            $('#dealer_phone_error').html(response.errors.dealer_phone);
                        }
                        else {
                            $('#dealer_phone_error').html('');
                        }
                        ...
                        if (response.errors.dealer_email) {
                            $('#dealer_email_error').html(response.errors.dealer_email);
                        }
                        else {
                            $('#dealer_email_error').html('');
                        }
                        ...
                        if (response.errors.dealer_username) {
                            $('#dealer_username_error').html(response.errors.dealer_username);
                        }
                        else {
                            $('#dealer_username_error').html('');
                        }
                        ...
                        if (response.errors.dealer_password) {
                            $('#dealer_password_error').html(response.errors.dealer_password);
                        }
                        else {
                            $('#dealer_password_error').html('');
                        }
    ...
    //MARK: Delete dealer
        ...
        $(".del_dealer").click(function () {
        $('#delete_dealer_id').val($(this).data('id'));
    });

    $('#yes_dealer_user').click(function () {
        $.ajax({
            url: base_url + 'admin/Settings/deletedealer',
            type: 'POST',
            data: {
                'id': $('#delete_dealer_id').val()
            },
            dataType: 'json',
            success: function (success) {
                location.reload();
...
e.preventDefault(); // prevent form submission until validation
    ...
    if ($('#loginusername').val().trim() === '') {
        $('#login_username_error').text('username field required.');
        isValid = false;
    }

    // Validate Password
    if ($('#loginpassword').val().trim() === '') {
        $('#login_password_error').text('Password field required.');
        isValid = false;
    }
    ...
    if(isValid){
        ...
        $.ajax({
            url: base_url + 'home/websitelogin',
            type: 'POST',
            data: {
...
e.preventDefault(); // prevent form submission until validation
    ...
    // Validate Password
    if ($('#loginusername').val().trim() === '') {
        $('#login_username_error').text('Mail is required.');
        isValid = false;
    }
    ...
    if(isValid){
        ...
        $.ajax({
            url: base_url + 'home/webforgotpassword',
            type: 'POST',
            data: {
                username: $('#loginusername').val(),
...
$(document).on('click', '.despatch', function () {
  ...
  var trackingId = $('#order_track').val().trim();
    $('#order_track_error').text('');
    if (trackingId === '') {
        $('#order_track_error').text('Please enter order Tracking ID').css('color', 'red');
        $('#order_track').focus();
        return false; 
    }
  $.ajax({
            url: base_url + 'admin/order/emaildespatch',
            type: 'POST',
            data: {
                'id': id,
            ...
            success: function (response) {
                if(response.success=='success'){
                    setTimeout(function () {
                        $('#successModal .modal-body').text(' email despatch successfully');
                        $('#successModal').modal('show');
...
$(document).on('click', '.convertorder', function () {
  var id = $('#hidden_order_id').val();
  var remarks = $('#remarks').val().trim();
    $('#order_remark_error').text('');
    if (remarks === '') {
        $('#order_remark_error').text('Please enter remarks').css('color','red');
        $('#remarks').focus();
        return false; 
    }
  $.ajax({
            url: base_url + 'admin/order/convertorder',   
            type: 'POST',
            data: {
                'id': id,
                'remarks': remarks
            ...
            success: function (response) {
                if(response.success=='success'){
                    setTimeout(function () {
                        $('#successModal .modal-body').text('order remark added successfully');
                        $('#successModal').modal('show');
                        $('#edit-order').modal('hide');
...
$(document).on('click', '.canceldespatch', function () {
  var id = $(this).attr('data-id');
  $.ajax({
            url: base_url + 'admin/Order/emailcanceldespatch',
            type: 'POST',
            data: {
                'id': id
            },
            ...
            success: function (response) {
                if(response.success=='success'){
                    setTimeout(function () {
                        $('#successModal .modal-body').text(' email undespatch successfully');
                        $('#successModal').modal('show');
                        $('#edit-order').modal('hide');
                        setTimeout(function () {
                            $('#successModal').modal('hide');
...
//MARK: search pending order
$(document).on('click', '#retail-search', function () {
  var type = $(this).attr('data-type');
  var name = $('#retail-name').val();
  var email = $('#retail-email').val();
  var orderno = $('#retail-orderno').val();
  var phone = $('#retail-phone').val();
  $.ajax({
            url: base_url + 'admin/Order/searchPendingOrder',
            type: 'POST',
            data: {
                'name': name,
                'orderno': orderno,
            ...
            success: function (response) {
                if(response.success=='success'){
                    if(response.type = 'rt')
                    {
                        $("#retail-orders tbody").html(response.html);
                    }
                }
            },
        })
...
//MARK: unpaid retail order
$(document).on('click', '#unpaid-search', function () {
  var type = $(this).attr('data-type');
  var name = $('#unpaid-name').val();
  ...
  $.ajax({
            ...
            success: function (response) {
                if(response.success=='success'){
                    if(response.type = 'rt')
                    {
                        $("#unpaid-orders tbody").html(response.html);
                    }
                }
            },
        })
...
//MARK: despatch retail order
$(document).on('click', '#despatch-search', function () {
  var type = $(this).attr('data-type');
  var name = $('#despatch-name').val();
  var email = $('#despatch-email').val();
  var orderno = $('#despatch-orderno').val();
  var phone = $('#despatch-phone').val();
  $.ajax({
            url: base_url + 'admin/Order/searchdespatchOrder',
            type: 'POST',
            data: {
                'name': name,
                'orderno': orderno,
            ...
            success: function (response) {
                if(response.success=='success'){
                    if(response.type = 'rt')
                    {
                        $("#despatch-retail tbody").html(response.html);
                    }
                }
            },
        })
...
// //76. get name search for report 
// $('#retail-name').on('change', function () {
//     var name = $(this).val();
//     var date = $('#retail-date').val();
//     loadRetailReportname(name,date);
// });


// function loadRetailReportname(name,date) {
//     $.ajax({
//         url: base_url + 'admin/Reports/getRetailReportname',
//         type: 'POST',
//         data: {
//             'name': name,
//             'date': date
...
// //77. get name search for report 
// $('#retail-phone').on('change', function () {
//     var name= $('#retail-name').val();
//     var date = $('#retail-date').val();
//      var phone = $(this).val();
//     loadRetailReportphone(name,date,phone);
// });


// function loadRetailReportphone(name,date,phone) {
//     $.ajax({
//         url: base_url + 'admin/Reports/getRetailReportphone',
//         type: 'POST',
//         data: {
//             'name': name,
//             'date': date,
