...
class Cart extends CI_Controller {
    ...
    public function cartview(){
       ...
       // print_r($data['customerdetails']); exit;
        ...
        $orders_id= $this->Homemodel->getorderid(); 
         $data['orderno'] = 'ORD' . date('Y') . $orders_id; 
    ...
    public function cart()
    {
       ...
       // ❌ Don't allow invalid quantity or price
...
//    $shippingcharge = isset($shippingcharge) ? $shippingcharge : 0;
    ...
    echo json_encode(['success' => 'success', 'data' => $cartitems, 'message'=>'product added to cart successfully','totalprice'=>$sumofprice,'total_amount'=>$total_amout,'weightcalculation'=>$this->Productmodel->getweightcalc($token),'shippingcharge'=>$shipping, 'ordertype'=>$this->session->userdata('order-type')]);
